...
fn test_pgm019_finding_details() {
    let findings = lint_fixture_rules("all-rules", &["V014__exclude_constraint.sql"], &["PGM019"]);
    let findings = normalize_findings(findings, "all-rules");
    insta::assert_yaml_snapshot!(findings);
}
...
fn test_pgm020_finding_details() {
    let findings = lint_fixture_rules("all-rules", &["V015__disable_trigger.sql"], &["PGM020"]);
    let findings = normalize_findings(findings, "all-rules");
    insta::assert_yaml_snapshot!(findings);
}

#[test]
fn test_pgm021_finding_details() {
    let findings = lint_fixture_rules("all-rules", &["V017__vacuum_full.sql"], &["PGM021"]);
    let findings = normalize_findings(findings, "all-rules");
    insta::assert_yaml_snapshot!(findings);
}
...
#[test]
fn test_pgm509_finding_details() {
    let findings = lint_fixture_rules("all-rules", &["V020__quoted_identifiers.sql"], &["PGM509"]);
    let findings = normalize_findings(findings, "all-rules");
    insta::assert_yaml_snapshot!(findings);
}
...
fn test_catalog_ops_set_drop_default_and_hash_index() {
    ...
    let findings = lint_fixture("catalog-ops", &["V003__defaults_and_hash.sql"]);
    let rule_ids: Vec<&str> = findings.iter().map(|f| f.rule_id.as_str()).collect();
    assert!(
        rule_ids.contains(&"PGM001"),
...
fn test_catalog_ops_hash_index_does_not_suppress_pgm501() {
    ...
    let findings = lint_fixture_rules("catalog-ops", &["V003__defaults_and_hash.sql"], &["PGM501"]);
    assert_eq!(
        findings.len(),
        1,
